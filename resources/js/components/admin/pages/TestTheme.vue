<template>
    ...
    <main class="w-full md:w-[calc(100%-256px)] md:ml-64 bg-gray-50 min-h-screen transition-all main">
        <div class="py-2 px-6 bg-white flex items-center shadow-md shadow-black/5 sticky top-0 left-0 z-30">
            <button @click="isActive" type="button" class="text-lg text-gray-600 sidebar-toggle">
                <i class="ri-menu-line"></i> Toggle menu
            </button>
...
const notificationTabClass = computed(() => {
    return {
        'text-blue-500': isActive.value,
        'border-b-blue-500': isActive.value,
        'hover:text-blue-500': !isActive.value,
    };
});

const orderTabClass = computed(() => {
    return {
        'bg-blue-500': isActive.value,
        'text-white': isActive.value,
        'hover:text-white': !isActive.value,
    };
});

const mainClass = computed(() => {
    return {
        'md:w-full': isActive.value,
        'md:ml-0': isActive.value,
    };
});

onMounted(() => {
    main.value = document.querySelector('.main');
    sidebarOverlay.value = document.querySelector('.sidebar-overlay');
    sidebarMenu.value = document.querySelector('.sidebar-menu');

    // Initial check and setup for responsiveness
    checkResponsive();
...
const checkResponsive = () => {
    if (window.innerWidth < 768) {
        closeSidebar(); // Ensure sidebar is closed initially for small screens
    }
};

const toggleSidebar = () => {
    main.value.classList.toggle('active');
    sidebarOverlay.value.classList.toggle('hidden');
    sidebarMenu.value.classList.toggle('-translate-x-full');
};

const closeSidebar = () => {
    main.value.classList.add('active');
    sidebarOverlay.value.classList.add('hidden');
    sidebarMenu.value.classList.add('-translate-x-full');
};
...
const handleDropdownToggle = (e, parent) => {
    e.preventDefault();
